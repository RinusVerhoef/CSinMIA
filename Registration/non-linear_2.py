...
def warp_label_with_transformix(label_img, transform_param_file, out_dir):
...
for pat_idx in range(NUM_TEST_PATIENTS):
    ...
    with TemporaryDirectory(prefix="bspline_topk_") as tmp:
        ...
        for i, moving_img in enumerate(top_images):
            ...
            affine_tp_file = write_transform_parameter_file_from_tmap(
                top_tmaps[i], work_dir / "affine_only_tp.txt")
            ...
            try:
                ...
                bspline_img, bs_metric, bs_tp_file = run_bspline_from_affine(
                    ...
                    affine_tmap=top_tmaps[i], work_dir=work_dir)
        ...
        fig1.suptitle(
            ...
            f"best {FUSION_SIZE} selected (by metric)", fontsize=14, y=0.98)
        ...
        for i, img in enumerate(final_images):
            ...
            title_txt = (f"Atlas {top_indices[i]:03d}\n"
                         f"Affine: {top_metrics[i]:.3f} | BS: {bs_str}\n"
                         f"{used_stage[i]} | {'SELECTED' if is_selected else 'not used'}")
    ...
    patient_result = {
        ...
        "test_index": pat_idx,
        "dice": dice, "jaccard": jacc, "hausdorff": hd, "rvd": rvd,
        "time_s": pat_time,
        "n_bs_used": n_bs_used, "n_bs_fail": n_bs_fail, "n_bs_worse": n_bs_worse,
